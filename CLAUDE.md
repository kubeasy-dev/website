...
- **Client-side**: Use `posthog.captureException(error)` from `posthog-js`
- Client-side uncaught errors are automatically captured via `capture_exceptions: true` in PostHog init
- Server-side request errors are captured via `onRequestError` in `instrumentation.ts`
- tRPC errors are captured via custom error middleware in `server/api/trpc.ts`
