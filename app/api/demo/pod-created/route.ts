...
/**
 * POST /api/demo/pod-created
 * Called by the CLI when the nginx pod is detected as running
 *
 * Request body:
 * {
 *   token: string;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isRealtimeConfigured && realtime) {
      ...
      await channel.emit("demo.pod_created", {
        timestamp: new Date(),
      });
    }
    ...
    logger.error("Demo pod-created error", {
      error: error instanceof Error ? error.message : String(error),
    });
    Sentry.captureException(error, {
      tags: { operation: "demo.pod_created" },
    });
    return NextResponse.json(
