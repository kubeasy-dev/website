...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get the Octokit instance for the kubeasy-dev organization
    const octokit = await app.getInstallationOctokit({
      owner: "kubeasy-dev",
    });

    // Search for discussions in the challenges repository
    ...
    const matchingDiscussion = discussions.find((discussion) => discussion.title === expectedTitle);
    ...
    return NextResponse.json({
      url: matchingDiscussion.html_url,
