...
export default function ChallengeCard({ challenge }: Readonly<{ challenge: Challenge }>) {
  ...
  const { data: progress } = useQuery(queries.userProgress.get(supabase, { challengeId: challenge.id }), {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className='h-full' ref={ref}>
